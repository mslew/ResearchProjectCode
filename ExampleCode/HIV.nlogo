...
;reset globals
;set infection chance to 50 and symptoms-show to 200.0
;check sliders for values
to setup-globals
...
to setup-people
  create-turtles initial-people ;create the amount of turtles as specified by slider
    [ setxy random-xcor random-ycor ;set random x and y coordinate
      set known? false ;set if they know they might be infected
      set coupled? false ;set in a relationship to be false
      set partner nobody ;no partners as of yet
      ifelse random 2 = 0 ;generate random number up to 2 if it equals zero the shape is person righty if not person lefty
      ...
      if infected?
        [ set infection-length random-float symptoms-show ] ;set infection length to random float
      ;assign values from submodels
      assign-commitment
...
to assign-color  ;; turtle procedure
  ifelse not infected? ;if not infected set color to green
    [ set color green ]
    [ ifelse known? ;else if they know they're infected set to red if ont set to blue
      [ set color red ]
      [ set color blue ] ]
end
...
;;;
to go
  if all? turtles [known?] ;if all turtles have known HIV stop simulation
    [ stop ]
  check-sliders ;;call the check sliders submodel
  ask turtles
    [ if infected?
        [ set infection-length infection-length + 1 ] ;if infected increment infection length by 1
      if coupled?
        [ set couple-length couple-length + 1 ] ] ;if coupled increment couple-length by 1
  ask turtles
    [ if not coupled?
        [ move ] ] ;if the turtle is not coupled we move it around
  ;; Righties are always the ones to initiate mating.  This is purely
  ;; arbitrary choice which makes the coding easier.
  ask turtles
    [ if not coupled? and shape = "person righty" and (random-float 10.0 < coupling-tendency) ;if turtle is not coupled and the shape is righty and we generate a random float is less than the
      [ couple ] ]                                                                            ;coupling tendency, we couple
  ask turtles [ uncouple ] ;uncouple
  ask turtles [ infect ] ;infect
  ask turtles [ test ] ;test
  ask turtles [ assign-color ] ;assign new color based on their status
...
to check-sliders ;for each slider we check and make sure the values havent changed. if they have we change the global variables
...
to couple  ;; turtle procedure -- righties only!
  let potential-partner one-of (turtles-at -1 0)
                          with [not coupled? and shape = "person lefty"] ;;couple the turtles with righty and lefty
  if potential-partner != nobody ;if potential-partner is not nobody
    [ if random-float 10.0 < [coupling-tendency] of potential-partner ;if random-float from 0-10 is less than coupling-tendency of potential-partner
      [ set partner potential-partner
        set coupled? true ;coupled is now true
        ask partner [ set coupled? true ] ;partner coupled is now true
        ask partner [ set partner myself ] ; partner itself
        move-to patch-here ;; move to center of patch
        ask potential-partner [move-to patch-here] ;; partner moves to center of patch
        set pcolor gray - 3 ;patch color is now green
        ask (patch-at -1 0) [ set pcolor gray - 3 ] ] ] ;ask patch to set color gray
end
...
to uncouple  ;; turtle procedure
  if coupled? and (shape = "person righty") ;if person is coupled and the person is a righty
    [ if (couple-length > commitment) or
         ([couple-length] of partner) > ([commitment] of partner) ;if couple-length is greater than commitment  or couple-length of partner is greater than commitment of partner
        [ set coupled? false ;coupled is now false
          set couple-length 0 ;couple length is 0
          ask partner [ set couple-length 0 ] ;make partner's couple-length to 0
          set pcolor black ;pcolor black
          ask (patch-at -1 0) [ set pcolor black ] ;set pcolor back to black
          ask partner [ set partner nobody ] ;set partner to nobody
          ask partner [ set coupled? false ] ;set partner coupled to false
          set partner nobody ] ] ;set partner to nobody
end
...
to infect  ;; turtle procedure
  if coupled? and infected? and not known? ;if couple and infected and not known
    [ if random-float 10 > condom-use or ;if random-float from 0-10 greater than condom-use or
         random-float 10 > ([condom-use] of partner) ;random float 0-10  greater than condom use of partner
        [ if random-float 100 < infection-chance ;if random-float 0-10 < infection chance
            [ ask partner [ set infected? true ] ] ] ] ;partner set infected true
end
...
to test  ;; turtle procedure
  if random-float 52 < test-frequency ;if random-float 0-52 is less than test-frequency
    [ if infected? ;if infected
        [ set known? true ] ] ;set the known to true
  if infection-length > symptoms-show ;if infection-length is greater than symptoms-show
    [ if random-float 100 < 5 ;if random-float 0-100 is less than 5
        [ set known? true ] ] ;set known to true
end
...
;;;
to-report %infected
  ifelse any? turtles ;if any turtles
    [ report (count turtles with [infected?] / count turtles) * 100 ] ;report infected turtles / turtles * 100 to get percentage
    [ report 0 ] ;if no turtles are infected report 0
end
