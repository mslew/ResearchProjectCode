...
to setup-people
  create-turtles initial-people
    [ setxy random-xcor random-ycor
      ...
      ;initially people are not cured, isolated, hospitalized, ambulance, infected, or susceptible
      set cured? false
      set isolated? false
      ...
      ;if continent = 1 set shape to 1. if not set shape to circle
      ifelse continent = 1
        [ set shape "square" ]
        [ set shape "circle" ]
      ...
      ;set chances of inoculation. if random-float is less than inoculation-chance then we set inoculated to true, susceptible false, and inoculated false
...
to setup-ambulance
  ...
  [
    ;random number of 1 and 2 and set the continent of that ambulance
    ifelse random 2 < 1
    [
      set continent 1
      setxy (- max-pxcor / 2) 0
    ...
    ;since this isnt a person, we set only the ambulance to be true
    set cured? false
    set isolated? false
    set hospitalized? false
    set infected? false
    ...
    ;set shape to person. color to yellow
    set shape "person"
    set color yellow
  ]
end

;assigning tendency of the turtles
...
to go
  if all? turtles [ not infected? ] ;stop procedure so it wont keep going
    [ stop ]
  ask turtles
    [ clear-count ]

  ask turtles ;if not isolated and not hospitalized and no ambulance then move the turtle
    [ if not isolated? and not hospitalized? and not ambulance?
        [ move ] ]

  ask turtles ;if not isolated and not hospitalized then infect
    [ if infected? and not isolated? and not hospitalized?
         [ infect ] ]

  ask turtles ;if not isolated and not hospitalized and infected and generate number between 0-100 is less that the turtles isolation tendency then isolate
    [ if not isolated? and not hospitalized? and infected? and (random 100 < isolation-tendency)
        [ isolate ] ]

  ask turtles ;if not isolated and not hospitalized and infected and generate number between 0-100 is less than the hospital-going-tendency then hospitalize
  ...
  ;maybe recover if infected
  ask turtles
    [ if infected?
       [ maybe-recover ]
    ]

  ;if isolated or hospitalized and cured unisolate
  ask turtles
    [ if (isolated? or hospitalized?) and cured?
        [ unisolate ] ]

  ;assign-color and calculate the r0
  ask turtles
    [ assign-color
      calculate-r0 ]
...
to move  ;; turtle procedure
  if travel?
  [
    ; random number between 1-100. if less than travel-tendency and not an ambulance we set x coordinate
...
;submodel to maybe recover
to maybe-recover
  set infection-length infection-length + 1 ;increment infection length by 1 same thing as infection-length++ I THINK
...
to infect  ;; turtle procedure
    ...
    if nearby-uninfected != nobody ;if nearby-uninfected is not equal to nobody.
    [
...
to calculate-r0
  ...
  let new-infected sum [ nb-infected ] of turtles ;new-infected is equal to the nb-infected
  let new-recovered sum [ nb-recovered ] of turtles ;new-recovered is sum of nb-recovered
  ...
  ifelse nb-infected-previous < 10 ;if nb-infected-previous is less than 10 set beta-n to 0. else set beta-n to (new-infected / nb-infected previous)
  [ set beta-n 0 ]
  [
  ...
  ifelse nb-infected-previous < 5 ;if nb-infected-previous is less than 5 set gamma to 0. else set gamma to (new-recovered / nb-infected-previous)
  [ set gamma 0 ]
  [
