...
to setup-people
  ...
  [
    ...
    if (random-float 100 < 5) ;random-float 100 gives random float below 100. if lower than 5 that agent gets infected
    [
      set infected? true
...
to go
  if all? turtles [ not infected? ] ;need this in order to stop. if all not infected then we stop the simulation
    [ stop ]

  ask turtles
    [ move ;has to be defined underneath
      clear-count ]

  ask turtles with [ infected? ]
    [ infect ;has to be defined underneath
      maybe-recover ]

  ask turtles
    [ assign-color ;has to be defined underneath
      calculate-r0 ]
...
to calculate-r0 ;R0: average secondary infections can you expect from a single infection.
